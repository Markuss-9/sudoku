...
console.log(sudoku3);
fs.writeFile("sudoku3.json", JSON.stringify(sudoku3, null, "\t"), (err) => {
	if (err) {
		console.error(err);
	}
});
console.log(`\\\\\\\\\\\\\\\\\\\\\\\\`);
console.log(sudoku);
fs.writeFile("sudoku.json", JSON.stringify(sudoku, null, "\t"), (err) => {
	if (err) {
		console.error(err);
	}
});
...
function duplicateNumberInBox(s, fieldX, fieldY) {
	var boxX = Math.floor(fieldY / 3) * 3;
	var boxY = Math.floor(fieldX / 3) * 3;

	// var number1 = [];
	// for (let i = boxY; i < boxY + 3; i++) {
	// 	number1 = [...number1, ...s[i].slice(boxX, boxX + 3)];
	// }
	// console.log(
	// 	number1.filter((item, index) => number1.indexOf(item) !== index)
	// 		.length !== 0
	// );
	// console.log(number1);

	numbers = new Array();
	for (var x = boxY; x < boxY + 3; x++) {
		for (var y = boxX; y < boxX + 3; y++) {
			if (s[x][y] !== 0) {
...
function generateRandomSudoku(numbers) {
	while (!sudoku_complete() || sudoku_invalid(sudoku)) {
		...
		fs.writeFile(
			"sudoku-test-before-solve.json",
			JSON.stringify(sudoku, null, "\t"),
			(err) => {
				if (err) {
					console.error(err);
				}
